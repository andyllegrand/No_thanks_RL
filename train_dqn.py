...
n_actions = 2 # take / pass
n_observations = num_players + max_card_num - min_card_num + 3
...
for i_episode in range(num_episodes):
    ...
    dqn_agent = DQN_Agent(memory, policy_net, 1)
    players = []
    games = initialize_games(games_per_epoch)
    flip_cards(games)
    ...
    while True:
      ...
      player.play_turn(player_games, player_num)
      turn_count += 1

      # update over flags
      update_over_flag(games)

      # update model (only player 1 is the DQN agent)
      if player_num == 1:
        optimize_model()
